...
void Camera::setPosition( QVector3D position ){
    ...
    hRot = 0.0f;
    vRot = 90.0f;
    center.setZ( center.x() + distance * sin(vRot) * cos(hRot) );
    center.setY( center.y() + distance * cos(vRot) );
    center.setZ( center.z() + distance * sin(vRot) * sin(hRot) );
}
...
QMatrix4x4 Camera::getMatrix(){
    ...
    center.setX( eye.y() + distance * cos(vRot) );
    center.setX( eye.z() + distance * sin(vRot) * sin(hRot) );
